...
import { getSelectedPreviousDate } from "../utils/getSelectedPreviousDate";

//This functions will manage the feed of news. Here will be all the fetchs needed based on filter we give him.
//currentLocation -> From where the action is executed (/, /new,/top,/search,/category)
  ...
  useEffect(() => {
    ...
    const getNewsData = async (url) => {
      try {
        setIsLoading(true);
        setError(undefined);
        //We check if the location is top. If it is we use the named fuction which will do a fetch and order
    ...
    //We do a switch conditional to check which location we are.
    switch (currentLocation) {
      ...
      case "top":
        ...
        //This function will give us the correct date because we send it the filterDate
        const dateToFetch = getSelectedPreviousDate(filterDate);
        getNewsData(
          `${process.env.REACT_APP_BACKEND}/news?date=${dateToFetch}`
        );
      ...
      case "search":
        ...
        if (sortFilter === "top") {
          const dateToFetch = getSelectedPreviousDate(filterDate);
          getNewsData(
            `${process.env.REACT_APP_BACKEND}/news?q=${text}&date=${dateToFetch}`
          );
        } else {
      ...
      case "category":
        ...
        if (sortFilter === "top") {
          ...
          currentCategoryName &&
            getNewsData(
              `${process.env.REACT_APP_BACKEND}/news?category=${currentCategoryName}&date=${dateToFetch}`
            );
        } else {
